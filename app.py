...
def execute_strategic_trade(line1_val, line2_val):
    try:
        # '교환 없음'을 제외한 실제 인덱스 추출
        indices = [v for v in [line1_val, line2_val] if v is not None]
        unique_indices = list(set(indices))
        
        if not unique_indices:
            st.toast("교환할 라인이 선택되지 않았습니다.", icon="ℹ️")
            return
        ...
        st.success(f"🔄 트레이드 완료: {', '.join([str(i)+'번' for i in unique_indices])} 라인이 교체되었습니다.")
    ...
    with col_form:
        ...
        if names_input:
            ...
            if len(name_list) >= 2:
                ...
                l_b = c2.selectbox("🔴 레드팀 팀장 선택", [n for n in name_list if n != l_a], index=1 if len(name_list)>1 else 0)
    ...
    with col_mid:
        if st.session_state.phase == 'auction':
            ...
            with st.form("bid_form", clear_on_submit=True):
                st.markdown("<p style='text-align:center;'>비공개 입찰 (0 ~ 100 Gold)</p>", unsafe_allow_html=True)
                c1, c2 = st.columns(2)
            ...
            st.markdown("#### 🔄 골드가 많이 남은 팀이 트레이드 진행 여부를 결정합니다")
            ...
            if is_same:
                st.warning("동일한 라인을 중복 선택했습니다.")
            
            if st.button("🔄 트레이드 실행", disabled=is_same):
                execute_strategic_trade(options[line1], options[line2])
                st.rerun()
